  ...
  try {
    ...
    const { data: request, error: createError } = await supabase
      ...
      .insert({
        ...
        mensaje:
          'Esta es una solicitud de prueba para validar el sistema de estados.',
        tipo: 1,
        prioridad: 'baja',
    ...
    console.log(
      `✅ Solicitud creada con éxito. ID: ${request.id}, Estatus: ${request.estatus}`
    );
        ...
        metadata: {
          ...
          status_history: [
            ...request.metadata.status_history,
            {
              ...
              to: 'en_proceso',
              timestamp: new Date().toISOString(),
              reason: 'Simulación interacción agente',
            },
          ],
        },
      })
    ...
    console.log(
      `✅ Transición automática exitosa. ID: ${updatedReq.id}, Estatus: ${updatedReq.estatus}`
    );
        ...
        metadata: {
          ...
          status_history: [
            ...updatedReq.metadata.status_history,
            {
              ...
              to: 'resuelto',
              timestamp: new Date().toISOString(),
              reason: 'Simulación cierre manual',
            },
          ],
        },
      })
    ...
    console.log(
      `✅ Cierre manual exitoso. ID: ${resolvedReq.id}, Estatus: ${resolvedReq.estatus}`
    );
    ...
    history.forEach((entry, index) => {
      console.log(
        `   [${index}] ${entry.from || 'START'} -> ${entry.to} | ${entry.timestamp} | ${entry.reason}`
      );
    });
