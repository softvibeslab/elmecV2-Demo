...
**Stack:** React Native 0.79.5, Expo 53, TypeScript 5.8, Supabase (PostgreSQL, Auth, Realtime)
...
- `app/(tabs)/chat/` - Chat section with dynamic routing
  - `index.tsx` - Chat list screen
  - `[roomId].tsx` - Individual chat room (dynamic route)
...
The app implements a client approval workflow:

**Approval States:** `pendiente` → `aprobado` | `rechazado`

**Features:**

- Admin approval management via `UserApprovalManagement.tsx`
- Zone-based filtering (Norte, Sur, Centro, Este, Oeste)
- Status tracking in `users.status_aprobacion` field
- Automated state flow with notification sync
- Visual indicators in admin dashboard

**Key Components:**

- `components/UserApprovalManagement.tsx` - Admin approval interface
- `components/AdminDashboard.tsx` - Dashboard with approval metrics

## Code Style

- Single quotes, 2-space indentation, trailing commas (es5)
- Use `@/` path alias for imports (configured in babel.config.js and tsconfig.json)
...
Spanish is the default and primary language. The app uses i18next with react-i18next:

- Configuration: `i18n/index.ts`
- Default locale: `es` (Spanish)
- Translations organized by feature (home, settings, barrenado, fresado)
- All UI text should use translation keys, not hardcoded strings

## File Upload System

The app supports both document and image uploads via `FileUploadComponent.tsx`:

- **Document Picker:** PDF, DOC, XLS, etc. (expo-document-picker)
- **Image Picker:** Camera and gallery (expo-image-picker)
- **Configurable limits:** maxFiles, allowedTypes, maxSizeInMB props
- **Permissions handled:** Camera and photos permissions in app.json
- **Upload utilities:** `utils/fileUpload.ts` handles file processing
...
- `20251210_group_chat_and_delivery.sql` - Group chat and message delivery system
- `20251211_message_delivery_functions.sql` - Delivery guarantee functions
- `20250114_add_rls_policies_requests.sql` - RLS policies for requests table
- `1761026070757_fix_rls_policies.sql` - RLS policy fixes
...
- `metro.config.js` has a temporary workaround for react-native-reanimated (blacklists it to resolve minSdkVersion conflict)
- `metro.config.js` disables package exports to fix TerminalReporter error with @expo/cli and Metro 0.83+
...
**Typed Routes:** Experiment enabled for type-safe navigation (`experiments.typedRoutes: true`)

**EAS Project:** `96c91e46-dc10-43b5-9cd9-4d91217fd50c` (for reference in EAS builds)

**Android Package:** `com.elmec.mfr` with minSdkVersion 24

**Plugins Configured:**

- expo-router (web origin for deep linking)
- expo-font
- expo-web-browser
- expo-image-picker (camera and photos permissions with Spanish text)
- expo-document-picker (iCloud container environment)
- expo-notifications (default channel configured)
