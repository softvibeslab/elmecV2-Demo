  ...
  private extractConfiguration(content: string): string {
    ...
    for (const line of lines) {
      if (line.match(/^#+\s+(Configuration|Settings|Config)/i)) {
        ...
        inConfig &&
        line.match(/^#+\s+(?!Configuration|Settings|Config)/)
      ) {
        break;
      }
...
export function useProgressiveDoc(docId: string) {
  ...
  const [stage, setStage] = useState<
    'loading' | 'basic' | 'enhanced' | 'complete'
  >('loading');
...
class RelatedDocumentFinder {
  ...
  private calculateSimilarity(
    features1: DocumentFeatures,
    features2: DocumentFeatures
  ): number {
    let score = 0;
    ...
    score +=
      (keywordIntersection / Math.max(1, features1.keywords.length)) * 0.4;
    ...
    const lengthRatio =
      Math.min(features1.length, features2.length) /
