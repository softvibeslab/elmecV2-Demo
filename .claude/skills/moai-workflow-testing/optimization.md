...
async function validatePerformanceBudget(
  ...
  if (metrics.duration > budget.maxTestDuration) {
    violations.push(
      `Duration ${metrics.duration}ms exceeds ${budget.maxTestDuration}ms`
    );
  }

  if (metrics.memoryUsage > budget.maxMemoryUsage) {
    violations.push(
      `Memory ${metrics.memoryUsage}MB exceeds ${budget.maxMemoryUsage}MB`
    );
  }
...
class TestMetricsCollector {
  async collectMetrics(testResult: TestResult) {
    const metrics = {
      ...
      browserMetrics: {
        domNodes: await testResult.page?.evaluate(
